...
namespace Skoruba.IdentityServer4.Admin.Helpers
{
	public static class StartupHelpers
	{
			...
			where TContext : DbContext
		{
			...
			services.AddDbContext<TContext>(options => options.UseSqlite(configuration.GetConnectionString(ConfigurationConsts.AdminConnectionStringKey), optionsSql => optionsSql.MigrationsAssembly(migrationsAssembly)));
		}
